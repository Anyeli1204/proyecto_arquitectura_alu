...
module tb_alu_frandom32;
  ...
  // =====================================
  reg [8*32-1:0] a_str;      // 16 caracteres
  reg [8*32-1:0] b_str;      // 16 caracteres
  reg [8*2-1:0]  op_str;     // 2 caracteres "00", "01", etc.
  reg [8*32-1:0] y_str;      // 16 caracteres
  ...
  initial begin
    ...
    // Abrir archivos
    fd_inputs  = $fopen("C:/Users/RODRIGO/alu/arquitectura_proyecto_alu.srcs/gen_random/data/tb_vectors_32_100000.mem", "r");
    fd_outputs = $fopen("C:/Users/RODRIGO/alu/arquitectura_proyecto_alu.srcs/gen_random/output/tb_expected_output32_100000.mem", "r");
    ...
    correct = correct + 1;
      ...
      if ((y[31] == expected_y[31]) &&  
          (y[30:23] == expected_y[30:23]) && 
          (y[22:6] == expected_y[22:6])) begin
        near_correct = near_correct + 1;
        $display("⚠️  Casi correcto en caso %0d: diferencia menor a 6 bits en mantisa.", total);
      end 
      // Comparación relajada: ignorar signo e inexacto
      else if ((y[30:0] === expected_y[30:0]) && (ALUFlags[4:1] === expected_flags[4:1])) begin
        relaxed_correct = relaxed_correct + 1;
        $display("ℹ️  Correcto (relajado) en caso %0d: diferencia solo en signo o bit de inexacto.", total);
      end
      else begin

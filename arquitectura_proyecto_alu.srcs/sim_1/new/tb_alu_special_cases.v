...
module tb_alu_special_cases;
  ...
  initial begin
    ...
    // Inf + Inf (mismo signo) = Inf ? overflow porque resultado es Inf
    op = OP_ADD; a = POS_INF; b = POS_INF;
    check("Inf + Inf", POS_INF, 0, 0, 1);  // ? overflow=1
    
    // Inf - Inf = NaN (INVALID, sin overflow)
    op = OP_SUB; a = POS_INF; b = POS_INF;
    check("Inf - Inf", QNAN, 1, 0, 0);
    
    // Inf + x = Inf ? overflow
    op = OP_ADD; a = POS_INF; b = ONE;
    check("Inf + 1", POS_INF, 0, 0, 1);  // ? overflow=1
    
    // Inf * 0 = NaN (INVALID, sin overflow)
    op = OP_MUL; a = POS_INF; b = POS_ZERO;
    check("Inf * 0", QNAN, 1, 0, 0);
    
    // Inf * 2 = Inf ? overflow
    op = OP_MUL; a = POS_INF; b = TWO;
    check("Inf * 2", POS_INF, 0, 0, 1);  // ? overflow=1
    ...
    // 1 / 0 = Inf (DIVIDE BY ZERO + OVERFLOW)
    op = OP_DIV; a = ONE; b = POS_ZERO;
    check("1 / 0", POS_INF, 0, 1, 1);  // ? div_zero=1, overflow=1
    ...
    // 1 / -0 = -Inf (DIVIDE BY ZERO, sin overflow)
    op = OP_DIV; a = ONE; b = NEG_ZERO;
    check("1 / -0", NEG_INF, 0, 1, 1);
